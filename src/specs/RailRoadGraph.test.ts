...
const addCityToGraph = (cityName: string) => {
  rrTestGraph.addCityToGraph(cityName);
};
...
const getPathLength = (cityNames: Array<string>) => {
  return rrTestGraph.getPathLength(cityNames);
};
...
const getNumTripsLimitByDistance = (
  startCityName: string,
  targetCityName: string,
  minDistance: number,
  maxDistance: number
) => {
  return rrTestGraph.getNumTripsLimitByDistance(
    startCityName,
    targetCityName,
    minDistance,
    maxDistance
...
const getNumTripsLimitByStops = (
  startCityName: string,
  targetCityName: string,
  minStops: number,
  maxStops: number
) => {
  return rrTestGraph.getNumTripsLimitByStops(
    startCityName,
    targetCityName,
    minStops,
    maxStops
...
describe("Test number of trips function", () => {
  ...
  test("Number of trips from t1 to t1 where distance <= 15", () => {
    expect(getNumTripsLimitByDistance("t1", "t1", 0, 15)).toBe(1);
  });

  test("Number of trips from t1 to t1 where numStops <= 4", () => {
    expect(getNumTripsLimitByStops("t1", "t1", 0, 4)).toBe(2);
  });

  test("Flag nonexistant path between t2 and t1", () => {
    expect(getNumTripsLimitByStops("t2", "t1", 0, 1)).toBe(badPathFlag);
  });
});
